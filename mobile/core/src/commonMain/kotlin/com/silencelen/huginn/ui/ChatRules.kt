 ...
 * which repeats the daemon's answer without reading it. Two things are wrong
 * with that answer and neither is the daemon's fault:
 *
 ...
 *  - a chat is only titled at its FIRST MESSAGE, so one that was created and
 *    never used has no title, no preview and nothing to tell it apart. On this
 *    host that is eighteen chats of fifty-three, thirteen of them consecutive:
 *    a block of identical rows reading "Untitled" over the same timestamp,
 *    which looks like a broken list rather than an empty one.
 */
object ChatRules {
    ...
    /** A chat with turns but no name. Something was said; nothing named it. */
    const val UNTITLED: String = "Untitled"

    /**
     * A chat nobody has said anything in.
     *
     * A DIFFERENT FACT from [UNTITLED], and the one the reader needs. Nothing
     * failed to name these — the daemon titles a chat from its first message and
     * there has not been one — so "Untitled" reads as a fault where "Empty chat"
     * reads as a state, and the thing to do about one is type in it or delete it.
     */
    const val EMPTY: String = "Empty chat"
    ...
    /** What a list row calls this chat. */
    fun listLabel(chat: Chat): String {
        title(chat.title)?.let { return it }
        val said = chat.turns > 0 || !chat.lastSnippet.isNullOrBlank()
        return if (said) UNTITLED else EMPTY
    }
}
